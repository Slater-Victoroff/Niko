"""Probe: for a trained FoundationModel checkpoint, how much does the ONE
shared cond bottleneck (context_cond_encoder(field_embedder(x_context)),
task-agnostic, the same module regardless of which task the batch is from)
actually know about a given task's own ground-truth physical params?

Extends probe_cond_to_params.py (single-task version, earlier this session)
to the foundation model's shared trunk: same idea (freeze everything, train
a tiny MLP on top of cond, supervise against ground truth never used in
training the backbone), but cond here was learned jointly across all three
tasks, not from a single task's own data alone -- this tests whether that
joint training entangled/diluted each task's own regime information, or
left it just as recoverable as the single-task case.

One probe per task (--task), since each task's own params differ in count
and meaning (rayleigh_benard/shear_flow: 2 log10 params; active_matter: 3,
one signed -> identity transform). rayleigh_benard/shear_flow also get the
snap-to-nearest-known-combo diagnostic (they train on a fixed small
param_subset); active_matter doesn't have one of those, so only continuous
...
TASK_TRANSFORMS = {
    "rayleigh_benard": ["log10", "log10"],       # Rayleigh, Prandtl
    "shear_flow": ["log10", "log10"],             # Reynolds, Schmidt
    "active_matter": ["identity", "identity", "identity"],  # L, zeta, alpha (alpha is negative)
}
...
def main():
    ...
    p.add_argument("--save-dir", default="/app/checkpoints/foundation_cond_to_params_probe")
    args = p.parse_args()
    ...
    # encoder/operator aren't needed for this probe (cond only depends on
    # field_embedder + context_cond_encoder), loaded only so the checkpoint's
    # completeness is implicitly verified and nothing here silently drifts
    # out of sync with what FoundationModel.forward actually computes cond from.
    ...
    print(f"Loaded {args.checkpoint} (epoch={ckpt['epoch']}, val_loss_by_task={ckpt['val_loss_by_task']}), "
          f"probing task='{args.task}', cond_dim={cond_dim}, backbone frozen.")
    ...
    def get_cond(xb):
        with torch.no_grad():
            ...
            return context_cond_encoder(x_context)
